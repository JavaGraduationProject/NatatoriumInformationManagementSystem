...
<html>
	<body>
		<display:table name="list"
			...
			<display:column title="员工" property="employeeName"/>
			<display:column title="月份">
				${row.yf}
				</display:column>
			<display:column title="月工资" property="fee1"/>
			<display:column title="奖励金额" property="fee2"/>
			<display:column title="惩罚金额" property="fee3"/>
			<display:column title="奖励惩罚说明" property="remark"/>
						<c:choose>
	...
	<script type="text/javascript">
		//更新字段内容
	function updateColumnemployeeId(employeeId,id){
		if(!confirm("确定要更新为"+employeeId+"吗?")){
			return false;
		}
		//var aaa = prompt("请输入操作意见或说明：","");
		var params={id:id,employeeId:employeeId};
		$.post("updateColumnemployeeId.html",params,function(
				result){
			result=eval("("+result+")");
